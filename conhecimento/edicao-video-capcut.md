...
- **Níveis:** voz com picos de −6 a −3 dB; música 12–18 dB abaixo da voz (clipe de música a −15/−20 dB durante a fala, −6/−8 dB nos trechos só de música); mix final ≈ −14 LUFS (algumas fontes: −16), pico real ≤ −1 dBTP. **Atenção:** o CapCut do usuário está com "Nível de volume desejado: Padrão (−23 LUFS)" em Configurações > Editar; normalizar com esse alvo deixa o áudio ~9 dB abaixo de −14. Subir o ganho do clipe (ou trocar o alvo, se o dono quiser; opções não verificadas) e medir o arquivo exportado (ex.: Youlean Loudness Meter). **Medido em 26/09/2026:** a exportação **não** normaliza — o arquivo sai com a mistura exata da linha do tempo (volumes dos clipes); o alvo −23 LUFS só vale para "Normalizar volume" aplicado no clipe. A conferência (`testar.bat conferir` / `video.conferir`) mede o LUFS do arquivo. Por isso o `video.preparar` mede o LUFS e o pico real de cada arquivo com som e o `video.planejar` já põe a voz no ganho que leva a ≈ −14 LUFS sem o pico passar de −1 dBTP (`plano_volume` em `config/video.json`).
...
## 12. Automação da edição (Fase 1, 25/09/2026)

Scripts em `rotinas/video/`, receitas em `receitas/*.json`, parâmetros em `config/video.json`, `config/capcut.json` e `config/exportacao.json`. Pastas: `Rotinas Ferreira\videos\bruto\<projeto>\` (nunca alterado), `trabalho\<projeto>\` (saídas), `exportado\`, `musicas\` (faixas-guia), `sons\` (efeitos sonoros próprios: whoosh, pop, clique, obturador, cortina).

- **Preparar o bruto (`video.preparar`):** inventário com avisos da §2, tomadas (T01, T02… globais), silêncios, transcrição local (faster-whisper "small", baixado no primeiro uso, ~480 MB), legendas `.srt` pelas regras da §5, batidas da faixa-guia, volume de cada arquivo com som (LUFS integrado e pico real, campo `volume`) e folhas de contato (3 quadros por tomada, 12 por folha). Fps variável: `normalizar_vfr` gera cópia em fps constante em `trabalho\` (o plano usa `caminho_edicao`).
- **Limites conhecidos da detecção:** o corte de cena do ffmpeg enxerga mudança de brilho; corte entre cores de brilho parecido pode passar (a tira de 3 quadros mostra; limiar `tomadas_limiar_cena`). Câmera na mão picota a tomada em várias: a IA junta passando `"tomadas": [...]` seguidas. Batidas: música de 150–180 BPM sai pela metade (75–90); chiado antes da música gera batidas falsas no começo; confiança de música real mixada fica ~0,5 (aviso só abaixo de 0,3).
- **Transcrição:** "R$" + número vira uma palavra no formato da loja (`R$229,99`); preço falado sem centavos vira `R$230,00`. Fala mais rápida que 17 caracteres/s não cabe sem dessincronizar: fica emendada e marcada "para revisar".
- **Receitas (R1–R12):** fiéis à §4.4, mas onde o guia passava do orçamento §4.1 ou de 1 efeito sonoro a cada 2–3 s, valeu a regra (R5 sem polaroid e obturador só nas fotos-chave; R6 com grão pelo ajuste de cor e sem vazamento de luz; R9 com "pop" só na animação; R10 sem tremor e brilho). R4 tem a abertura em dois blocos para chegar aos 10 s. Abertura pelo clímax (#21) dura 0,7–1,2 s. R2, R6 e R11 têm texto opcional de re-gancho (vídeo > 15 s).
- **Plano (`video.planejar`):** a IA escolhe tomadas e textos; o script monta a linha do tempo e valida (gancho com texto no quadro 1, duração da receita e do destino, troca a cada 1–3 s, zona segura, tempo e tamanho de texto, orçamento, ≤ 3 flashes/s, preço, corte na batida ±1 quadro). **Violação bloqueia** (o rascunho não é gerado). Clipe com fala: `volume_voz_db` da receita + ganho até −14 LUFS pelo `volume` do bruto (teto: pico real −1 dBTP; aviso "picos altos" quando o pico limita). Jump cut usa silêncios do ffmpeg e buracos ≥ 0,3 s entre palavras; palavra transcrita nunca é cortada. Largura de texto estimada (0,58 × tamanho; maiúscula 0,70) até calibrar com a régua.
- **Parcelas no texto:** padrão "3x sem juros" (formato do exemplo da R2). O valor da parcela ("3x de R$76,66 sem juros") só com `parcela_mostrar_valor: true`, depois de o dono confirmar como arredonda o centavo (`parcela_arredondamento`).
- **Rascunho (`video.rascunho`):** gerador próprio clonando o gabarito "0925" (avaliação em `capcut-avaliacao-ferramentas.md`). CapCut fechado, backup antes, validação antes de gravar, nome repetido vira "(2)". Efeitos e transições da biblioteca **não** entram: vão para o `relatorio_rascunho.txt` como acabamento manual (licença §7 e risco de mídia perdida). A faixa-guia entra **muda** (só marca o ritmo; a música final vai pelo Instagram). Quadro congelado (R9) fica manual.
- **Exportação e conferência (`video.exportar`, `video.conferir`):** por enquanto a exportação é um clique do usuário com o preset da §8 (instruções geradas); o script espera o `.mp4` novo e confere duração, 1080×1920, fps, H.264, taxa de bits, áudio, −14 LUFS ± 2, pico ≤ −1 dBTP, tamanho (stories ~100 MB) e SDR. Vídeo exportado sem música de propósito reprova em áudio/volume/pico — a dica explica que pode ignorar. Sem `rascunho` no pedido, nome do projeto a abrir e duração esperada vêm do último `relatorio_rascunho.txt` do projeto (o `plano.json` pode ter sido trocado por outro planejar; teste da skill de 25/09/2026).
- **Achados no teste da skill (25/09/2026), já tratados:** plano com violação ia para `plano.json` e apagava o bom — agora vai para `plano-com-violacoes.json`; sem transcrição, a voz de blocos que a receita não marca como fala entra muda e `legendas: true` dá 0 legendas — continua assim, mas agora com aviso no plano.
- **Auditoria de 25/09/2026:** a cópia em fps constante se chama `trabalho\<projeto>\cfr\<nome>-<ext>-cfr30.mp4` e tem ao lado `<cópia>.origem.json` (refeita se o original mudar); a conferência deduz do plano se o arquivo sai **mudo** ou só com efeitos (não reprova áudio/volume nesse caso; reprova se sair com som, ex.: faixa-guia esquecida ligada); `video.rascunho` em ensaio não mexe no relatório do projeto; a lista `manual` do rascunho é o acabamento completo (sem o punch-in, que o rascunho já aplica pela escala); gabarito não encontrado = o "0925" não está nos rascunhos: informar outro com `gabarito`.
