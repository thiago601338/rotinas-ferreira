...
- **Ensaios de 26/09/2026 00:33 e 00:34:** às 00:33 o **Instagram abriu deitado** (tela 1600×900, layout de tablet com abas na lateral e "Criar" na barra) e "Adicionar ao story" levou a uma tela preta; a mensagem dizia "toquei 3 vez(es)" mas foi 1 (agora conta os toques de verdade). Depois de abrir o Instagram o fluxo espera até `espera_em_pe_s` (10 s) a tela ficar em pé e, se não ficar, para com "o Instagram abriu deitado…". Às 00:34 (em pé) a **música passou inteira** ("Fool Me Once", ✓ Concluir) e o fluxo trocou para as mídias 2 e 3 pela faixa de miniaturas; parou na figurinha: o painel de figurinhas tem a mesma busca (`row_search_edit_text`); com "link" digitado aparece a seção "Figurinhas" com `asset_item` de descrição **"Figurinha de link"** (l minúsculo) e a seção GIPHY ("Figurinha de GIF"). O seletor procurava "Link" (maiúsculo): agora `figurinha_link` começa por "Figurinha de link"; `buscar_figurinha` usa `row_search_edit_text` e perdeu o EditText genérico; `concluir_figurinha` aceita "Concluir". Tela conferida em `tests/test_seletores_448.py` (`figurinhas_busca_link.xml`). A tela da figurinha foi vista no ensaio seguinte (abaixo).
- **Ensaios de 26/09/2026 00:40 e 00:45 (parou em `figurinha_3`):** "Figurinha de link" abriu a tela **"Adicionar link"**: voltar (`link_sticker_list_cancel_button`), título `link_sticker_list_title`, **Concluir** (`link_sticker_list_done_button`, texto "Concluir"), campo **URL** `link_sticker_list_web_url_edit_text` (o link `wa.me/…` foi digitado e conferido sem `https://`), aviso com "Ver prévia", e a linha **"Personalizar o texto da figurinha"** (`link_sticker_custom_cta_row`, com "o"; o seletor esperava "Personalizar texto da figurinha"). Corrigidos `campo_url`, `personalizar_texto`, `concluir_figurinha` pelo id; `campo_texto_figurinha` ganhou um padrão de id (`link_sticker…cta|custom…edit_text`) antes do EditText nº 2 — o campo do texto só aparece depois de tocar em "Personalizar" e ainda não foi visto. Tela conferida em `tests/test_seletores_448.py` (`adicionar_link.xml`).
- **Ensaio de 26/09/2026 01:11 (parou na conferência da figurinha):** URL e texto "Comprar" digitados e conferidos, Enter, "Concluir" → a figurinha **"COMPRAR"** apareceu na foto (print), mas no XML do 448 ela é só um item genérico com descrição **"Figurinhas. Toque e mantenha pressionado para reposicionar. Use dois dedos para girar ou redimensionar."** (dentro de `capture_interactive_drawable_container`), sem o texto nem o link; a conferência procurava "wa.me"/o texto e parou. A música em "Somente música" não cria esse item. Agora a conferência **conta os itens `figurinha_na_tela` antes e depois** de pôr a de link (tem que aumentar); se não aumentar, ainda aceita "wa.me"/o texto no XML (outras versões) e, sem isso, para. Tela conferida (`editor_com_figurinha.xml`).
