...
| "a busca de música 'fashion' não trouxe nenhuma faixa" | Nada foi publicado. Ver `busca_musica_*.png/.xml`: se a lista aparece no print, é o seletor `faixas_musica` que não bate com o XML (em 26/09 era o `resource-id` sem prefixo das telas Compose). Ajustar em `config/bluestacks.json`, conferir com a tela real em `tests/test_seletores_448.py` e registrar em `conhecimento/stories.md`. |
